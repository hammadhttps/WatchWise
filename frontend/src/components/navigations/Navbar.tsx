...
import { LogOut, User, Settings, ChevronDown, Bookmark, Film, Star } from 'lucide-react';
import AIMovieMatch from '../ai/AIMovieMatch';
...
const Navbar = () => {
  ...
  const { results: searchResults } = useMovieSearch(searchTerm);
  ...
  const navLinks = [
    ...
    { label: 'My Watchlist', to: '/profile?tab=watchlist' },
  ];
  ...
  const activeLink =
    ...
    : pathname.startsWith('/anime') ? 'Anime Hub'
    : pathname.startsWith('/trending') ? 'Trending'
    : pathname.startsWith('/coming-soon') ? 'Coming Soon'
    : pathname.startsWith('/profile') ? 'My Watchlist'
    : '';
    ...
    <>
          ...
          <ul className="hidden lg:flex items-center gap-1 list-none flex-1">
            {navLinks.map(({ label, to }) => (
              <li key={label}>
                <Link
                  to={to}
                  className={`block text-[13.5px] font-medium whitespace-nowrap px-[10px] py-[6px] rounded-md transition-all duration-150 ${
                    activeLink === label
          ...
          <div className="flex items-center gap-3.5 flex-shrink-0 ml-auto">
            ...
            <div className="relative hidden sm:flex items-center">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Escape') { setSearchTerm(''); setIsSearchOpen(false); searchRef.current?.blur(); } }}
              ...
              {isSearchOpen && searchTerm.trim().length >= 2 && (
                <div className="absolute right-0 top-[44px] w-[360px] bg-[#0a1628] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                  {searchResults.length === 0 ? (
                    <p className="px-4 py-3.5 text-[13px] text-white/40">No movies found for "{searchTerm.trim()}"</p>
                  ) : (
                    searchResults.map(movie => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-white/[0.07] transition-colors cursor-pointer text-left"
                      >
                        <div className="w-9 h-[54px] rounded-[5px] overflow-hidden flex-shrink-0 bg-[#112055] flex items-center justify-center">
                          {movie.poster_path ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13.5px] font-semibold text-white truncate">{movie.title}</p>
                          <p className="text-[11.5px] text-white/40">{movie.release_date?.split('-')[0] || '—'}</p>
                        </div>
                        {movie.vote_average > 0 && (
                          <span className="flex items-center gap-1 text-[11.5px] font-semibold text-yellow-400 flex-shrink-0">
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 bg-[#213C51] border border-white/25 text-white text-[14.5px] font-bold px-3.5 py-1.5 rounded-md cursor-pointer whitespace-nowrap hover:bg-white/20 hover:border-white/40 transition-all"
            >
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-[#112055] border border-white/[0.08] rounded-xl shadow-xl overflow-hidden">
                    ...
                    <div className="py-1">
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-[13.5px] text-white/80 hover:bg-white/10 transition-colors cursor-pointer"
                      >
                        <User size={16} />
                        <span>My Profile</span>
                      </button>
                      <button
                        onClick={() => { setIsUserMenuOpen(false); navigate('/profile?tab=watchlist'); }}
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-[13.5px] text-white/80 hover:bg-white/10 transition-colors cursor-pointer"
                      >
                        <Bookmark size={16} />
      ...
      {isMobileMenuOpen && (
        <div className="fixed top-[68px] left-0 right-0 z-40 bg-[#134686] border-t border-white/10 lg:hidden">
            ...
            {navLinks.map(({ label, to }) => (
              <li key={label}>
                <Link
                  ...
                  className={`block px-5 py-[11px] text-[14px] font-medium transition-all ${
                    activeLink === label
          ...
          <div className="px-5 pt-2.5 pb-4">
            <button
              onClick={() => { setIsMobileMenuOpen(false); setIsAIMatchOpen(true); }}
              className="w-full flex items-center justify-center gap-2 bg-white/10 border border-white/25 text-white text-[13.5px] font-semibold py-2.5 rounded-md hover:bg-white/20 transition-all cursor-pointer"
            >
              ✨ AI Movie Match
            </button>
          </div>
      ...
      <AIMovieMatch open={isAIMatchOpen} onClose={() => setIsAIMatchOpen(false)} />
    </>
  );
};
